...
function filenames = delete_wrong_images(filenames)

% Check whether the last four digits are a number and remove images that do
% not belong to this class --> put this in a function
% remove_non_molo_images
...
for i = 1:length(filenames)
   ...
   first_split = strsplit(filenames(i).name,'_');
   hilf = (first_split{end});
   hilf = strsplit(hilf,'.');
  
   % check whether all last four digits are numbers
   if ~all(isstrprop(hilf{1},'digit'))
...
%                 saveas(fig,strcat(handles.path,num2str(i),'.png'))
    ...
    if isfield(handles,'chemical_integrities_loaded')
       ...
       if isfield(handles.molo_algo_data.(handles.algorithm)(1,1),'molo_offset')
           ...
           % this seems to be too much here...
...
% here I need to calculate the damping constant

damping_constants = zeros(length(handles.Norm_Lower_ind_confirmed(1):handles.Norm_Lower_ind_confirmed(2)),1)

j = 1
for i = handles.Norm_Lower_ind_confirmed(1):handles.Norm_Lower_ind_confirmed(2)
...
for i = 1:length(algorithms)
    ...
    if isfield(handles.molo_algo_data.(algorithms{i})(1,1),'signal')
        ...
        export_data_to_csv('signal','MoloSignal a.u.',handles,algorithms{i});
        export_data_to_csv('background','Background Intensity a.u.',handles,algorithms{i})
        export_data_to_csv('surface_mass_densities_mod','\Gamma_\Delta  [pg/mm^2]',handles,algorithms{i})
...
% --------------------------------------------------------------------
function Select_Molo_Areas_Callback(hObject, eventdata, handles)
% hObject    handle to Select_Molo_Areas (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
