...
% specified by algorithm.
    ...
    for j = 1:size(handles.molo_algo_data.(algorithm),1)
         ...
         for k = 1:size(handles.molo_algo_data.(algorithm),2)
             ...
             if k == size(handles.molo_algo_data.(algorithm),2)
                 ...
                 h(j) =  plot(handles.time,handles.molo_algo_data.(algorithm)(j,k).background,'Linewidth',1,'Color',styles(j));
               
             else
               
               plot(handles.time,real(handles.molo_algo_data.(algorithm)(j,k).background),'Linewidth',1,'Color',styles(j));
    ...
    csvwrite_with_headers(strcat(handles.path,'/Evaluation/csv_files/', handles.experiment_name, '_', algorithm, '_background.txt'),signals_csv,header);
