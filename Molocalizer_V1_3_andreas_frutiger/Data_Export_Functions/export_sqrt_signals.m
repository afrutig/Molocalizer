...
% specified by algorithm.
    ...
    for j = 1:size(handles.molo_algo_data.(algorithm),1)
         ...
         for k = 1:size(handles.molo_algo_data.(algorithm),2)
             ...
             if k == size(handles.molo_algo_data.(algorithm),2)
                 ...
                 h(j) =  plot(handles.time,handles.molo_algo_data.(algorithm)(j,k).sqrt_signal,'Linewidth',1,'Color',styles(j));
               
             else
               
               plot(handles.time,real(handles.molo_algo_data.(algorithm)(j,k).sqrt_signal),'Linewidth',1,'Color',styles(j));
    ...
    name = strcat(handles.path,'/Evaluation/Plots/', handles.experiment_name, '_', algorithm, '_sqrt_signals.png');
    
    print(figure1,name,'-dpng')
    
    csvwrite_with_headers(strcat(handles.path,'/Evaluation/csv_files/', handles.experiment_name, '_', algorithm, '_sqrt_signals.txt'),signals_csv,header);
