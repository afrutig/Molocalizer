function export_norm_signals(handles,algorithm)
...
% specified by algorithm.
    ...
    for j = 1:size(handles.molo_algo_data.(algorithm),1)
         ...
         for k = 1:size(handles.molo_algo_data.(algorithm),2)
             ...
             signals_csv(:,index) = handles.molo_algo_data.(algorithm)(j,k).norm_signal;
             
             
             if k == size(handles.molo_algo_data.(algorithm),2)
                 
                 h(j) =  plot(handles.time,handles.molo(j,k).norm_signal,'Linewidth',1,'Color',styles(j));
               
             else
               
               plot(handles.time,real(handles.molo(j,k).norm_signal),'Linewidth',1,'Color',styles(j));
    ...
    name = strcat(handles.path,'/Evaluation/Plots/', algorithm, '_norm_signals.png');
    
    print(figure1,name,'-dpng')
    
    csvwrite_with_headers(strcat(handles.path,'/Evaluation/csv_files/', algorithm, '_norm_signals.txt'),signals_csv,header);
