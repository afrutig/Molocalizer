function export_chemical_integrities(handles,algorithm)
    ...
    ylabel('Chemical Integrity [uM]');
    
    name = strcat(handles.path,'/Evaluation/Plots/', algorithm, '_chemical_integrities.png');
    
    print(figure1,name,'-dpng')
    
    csvwrite_with_headers(strcat(handles.path,'/Evaluation/csv_files/', algorithm, '_chemical_integrities.txt'),chemical_integrities_csv,header);
    save(strcat(handles.path,'/Evaluation/mat_files/', algorithm, '_chemical_integrities.mat'),'chemical_integrity_matrix')
